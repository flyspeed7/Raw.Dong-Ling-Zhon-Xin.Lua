...
local Window = Fluent:CreateWindow({
    Title = game:GetService("MarketplaceService"):GetProductInfo(16732694052).Name .." | 冬凌汉化",
    SubTitle = "群:884776077",
    TabWidth = 160,
...
local function updateShadowCount()
    ...
    shadowCountLabel.Text = "阴影面积: " .. count
end
...
local Tabs = { -- https://lucide.dev/icons/
    ...
    Exclusives = Window:AddTab({ Title = "美化", Icon = "heart" }),
    Main = Window:AddTab({ Title = "主要的", Icon = "list" }),
    Items = Window:AddTab({ Title = "其他", Icon = "box" }),
    Teleports = Window:AddTab({ Title = "传送", Icon = "map-pin" }),
    Misc = Window:AddTab({ Title = "功能", Icon = "file-text" }),
    Trade = Window:AddTab({ Title = "设置", Icon = "gift" })
}
...
do
    ...
    -- // Exclusives Tab // --
    local sectionExclus = Tabs.Exclusives:AddSection("美化")
    local CountShadows = Tabs.Exclusives:AddToggle("CountShadows", {Title = "抛竿美化", Default = false })
    ...
    local RodDupe = Tabs.Exclusives:AddToggle("RodDupe", {Title = "深渊鱼竿", Default = false })
    ...
    -- // Main Tab // --
    local section = Tabs.Main:AddSection("自动钓鱼")
    local autoCast = Tabs.Main:AddToggle("autoCast", {Title = "自动抛竿", Default = false })
    ...
    local autoShake = Tabs.Main:AddToggle("autoShake", {Title = "自动摇动", Default = false })
    ...
    local autoReel = Tabs.Main:AddToggle("autoReel", {Title = "自动摇摆", Default = false })
    ...
    local FreezeCharacter = Tabs.Main:AddToggle("FreezeCharacter", {Title = "不可移动", Default = false })
    ...
    -- // Mode Tab // --
    local section = Tabs.Main:AddSection("自动模式设置")
    ...
    -- // Sell Tab // --
    local section = Tabs.Items:AddSection("出售")
    ...
    -- // Treasure Tab // --
    local section = Tabs.Items:AddSection("修复/获取")
    Tabs.Items:AddButton({
        Title = "杰克骨头",
        Callback = function()
    ...
    -- // Teleports Tab // --
    local section = Tabs.Teleports:AddSection("事件/传送")
    ...
    -- // Character Tab // --
    local section = Tabs.Misc:AddSection("玩家")
    local WalkOnWater = Tabs.Misc:AddToggle("WalkOnWater", {Title = "水上行走", Default = false })
    ...
    local ToggleNoclip = Tabs.Misc:AddToggle("ToggleNoclip", {Title = "穿墙", Default = false })
    ToggleNoclip:OnChanged(function()
        Noclip = Options.ToggleNoclip.Value
    end)

    -- // Misc Tab // --
    local section = Tabs.Misc:AddSection("功能")
    local BypassRadar = Tabs.Misc:AddToggle("BypassRadar", {Title = "开启 鱼雷达", Default = false })
    ...
    local BypassGPS = Tabs.Misc:AddToggle("BypassGPS", {Title = "开启 GPS", Default = false })
    ...
    local RemoveFog = Tabs.Misc:AddToggle("RemoveFog", {Title = "去雾气", Default = false })
    ...
    local DayOnly = Tabs.Misc:AddToggle("DayOnly", {Title = "天亮", Default = false })
    ...
    local HoldDuration = Tabs.Misc:AddToggle("HoldDuration", {Title = "持续时间0", Default = false })
    ...
    local DisableOxygen = Tabs.Misc:AddToggle("DisableOxygen", {Title = "无氧气", Default = true })
    ...
    local JustUI = Tabs.Misc:AddToggle("JustUI", {Title = "自适应 UI", Default = true })
    ...
    local IdentityHiderUI = Tabs.Misc:AddToggle("IdentityHiderUI", {Title = "修改数据", Default = false })    
    IdentityHiderUI:OnChanged(function()
        while Options.IdentityHiderUI.Value == true do
            if UserPlayer:FindFirstChild("streak") then UserPlayer.streak.Text = "" end
            if UserPlayer:FindFirstChild("level") then UserPlayer.level.Text = "等级: 1" end
            if UserPlayer:FindFirstChild("level") then UserPlayer.user.Text = "我是傻逼" end
            local hud = LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("hud"):WaitForChild("safezone")
            if hud:FindFirstChild("coins") then hud.coins.Text = "0 C$" end
            if hud:FindFirstChild("lvl") then hud.lvl.Text = "1 LVL" end
            task.wait(0.01)
        end
    end)
